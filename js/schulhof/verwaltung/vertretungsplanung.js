function cms_vplan_schulstunden_laden_von() {
  ...
  if (!fehler) {
    if (!cms_check_ganzzahl(tag, 1, 31)) {
      fehler = true;
    }
    if (!cms_check_ganzzahl(monat, 1, 12)) {
      fehler = true;
    }
    if (!cms_check_ganzzahl(jahr, 0)) {
      fehler = true;
    }
  }
  ...
  if (fehler) {
    ...
    function anfragennachbehandlung_vplanstunden(rueckgabe) {
      if (rueckgabe.match(/^<option/) || rueckgabe == "") {
        ...
        ('<p class="cms_notiz">Fehler beim Laden der Schulstunden.</p>');
      }
    }
  ...
  if (fehler) {
    ...
    function anfragennachbehandlung(rueckgabe) {
      if (rueckgabe.match(/^<option/) || rueckgabe == "") {
        ...
        ('<p class="cms_notiz">Fehler beim Laden der Klassen.</p>');
      }
    }
...
function cms_datumspeichern(tag, monat, jahr, art) {
  ...
  if (art != "a" && art != "k1" && art != "k2" && art != "l" && art != "r" && art != "ks") {
    fehler = true;
  }
...
function cms_ausplanung_speichern() {
  ...
  if (!cms_check_ganzzahl(vonT, 1, 31)) {
    fehler = true;
  }
  if (!cms_check_ganzzahl(vonM, 1, 12)) {
    fehler = true;
  }
  if (!cms_check_ganzzahl(vonJ, 0)) {
    fehler = true;
  }
  if (!cms_check_ganzzahl(bisT, 1, 31)) {
    fehler = true;
  }
  if (!cms_check_ganzzahl(bisM, 1, 12)) {
    fehler = true;
  }
  if (!cms_check_ganzzahl(bisJ, 0)) {
    fehler = true;
  }
  if (!cms_check_ganzzahl(vonS, 0) || !cms_check_ganzzahl(bisS, 0)) {
    fehler = true;
  }
  ...
  if (folge != "k" && folge != "e" && folge != "u") {
    fehler = true;
  }
  if (art != "l" && art != "r" && art != "k" && art != "s") {
    fehler = true;
  }
  if (art == "l" && grund != "dv" && grund != "k" && grund != "kk" && grund != "p" && grund != "b" && grund != "ex" && grund != "s") {
    fehler = true;
  } else if (art == "r" && grund != "b" && grund != "p" && grund != "k" && grund != "s") {
    fehler = true;
  } else if (art == "k" && grund != "ex" && grund != "sh" && grund != "p" && grund != "bv" && grund != "k" && grund != "s") {
    fehler = true;
  }
...
function cms_ausplanung_loeschen_anzeigen(name, id, art, rueck) {
  var zusatzknopf = "";
  if (rueck == "j") {
    zusatzknopf = ' <span class="cms_button_nein" onclick="cms_ausplanung_rueckabwicklung(\'' + id + "', '" + art + "')\">Ausplanung rückabwickeln</span>";
  }
...
function cms_ausplanung_rueckabwicklung_naechstestunde() {
  if (rueckkonflikte.length > 0) {
    ...
    if (aktkonflikt[1] == "") {
      aktstdart = "<b>Geplante Änderung:</b><br>";
    } else {
      aktstdart = "<b>Vormerkung:</b><br>";
    }
    if (aktkonflikt[10] == "e") {
      aktstdvorher += " (Entfall)";
    }
    if (aktkonflikt[10] == "z") {
      aktstdvorher = "Zusatzstunde";
    }
    if (aktkonflikt[1] == "") {
    ...
    if (document.getElementById("cms_vplanklasse_datum_T")) {
      cms_vplan_alles_neuladen();
    }
...
function cms_vplan_konflikte_liste(sortierung, sortierrichtung) {
  if (document.getElementById("cms_vplankonflikte_sortierrichtung")) {
    var sortierrichtungsalternative = document.getElementById("cms_vplankonflikte_sortierrichtung").value;
  } else {
    var sortierrichtungsalternative = "0";
  }
  var sortierrichtung = sortierrichtung || sortierrichtungsalternative;
  if (document.getElementById("cms_vplankonflikte_sortierung")) {
    var sortierungsalternative = document.getElementById("cms_vplankonflikte_sortierung").value;
  } else {
    var sortierungsalternative = "s";
  }
  ...
  if (!cms_check_toggle(sortierrichtung)) {
    fehler = true;
  }
...
function cms_vplan_konflikte_planwahl() {
  ...
  if (planart != "l" && planart != "r" && planart != "k" && planart != "s") {
    fehler = true;
  }
...
function cms_vplan_konflikte_plan() {
  ...
  if (!cms_check_ganzzahl(zweittag, 1, 31)) {
    fehler = true;
  }
  if (!cms_check_ganzzahl(zweitmonat, 1, 12)) {
    fehler = true;
  }
  if (!cms_check_ganzzahl(zweitjahr, 0)) {
    fehler = true;
  }
  if (!cms_check_toggle(regelplan)) {
    fehler = true;
  }
...
function cms_vplan_wochenplan_l(details) {
  ...
  if (!cms_check_ganzzahl(lehrer, 0)) {
    fehler = true;
  }
...
function cms_vplan_wochenplan_k() {
  ...
  if (!cms_check_ganzzahl(klasse.substr(1), 0)) {
    fehler = true;
  }
  if (klasse.substr(0, 1) != "s" && klasse.substr(0, 1) != "k") {
    fehler = true;
  }
...
function cms_vplan_stunde_ueberschreiben_konfliktanzeigen(art) {
  ...
  var zusatzmeldung = "";
  if (art.match(/L/)) {
    zusatzmeldung += ", Lehrer";
  }
  if (art.match(/R/)) {
    zusatzmeldung += ", Raum";
  }
  if (art.match(/K/)) {
    zusatzmeldung += ", Klasse";
  }
  if (art.match(/S/)) {
    zusatzmeldung += ", Stufe";
  }
...
function cms_vplan_freieressourcen_laden(modus, uid, kid, nr) {
  ...
  var kid = kid || "-";
  var fehler = false;
  if (!cms_check_ganzzahl(uid, 0) && uid != "-") {
    fehler = true;
  }
  if (!cms_check_ganzzahl(kid, 0) && kid != "-") {
    fehler = true;
  }
  if (modus != "k" && modus != "l" && modus != "d") {
    fehler = true;
  }
  if ((modus == "k" || modus == "l") && kid == "-" && uid == "-") {
    fehler = true;
  }
  if (!fehler) {
    if (modus == "k" || modus == "l") {
...
function cms_stundendetails_laden(uid, kid, datum, uhrzeit, anzeigen) {
  ...
  if (!datum.match(/^[0-9]{2}\.[0-9]{2}\.[0-9]{4}$/) && datum != "-") {
    fehler = true;
  }
  if (!uhrzeit.match(/^[0-9]{2}\:[0-9]{2}$/) && uhrzeit != "-") {
    fehler = true;
  }
  if (uid == "-" && kid == "-") {
    if (datum == "-" || uhrzeit == "-") {
  ...
  if (datum != "-" && uhrzeit != "-" && !fehler) {
    ...
    if (!cms_check_ganzzahl(std, 0, 23)) {
      fehler = true;
    }
    if (!cms_check_ganzzahl(min, 0, 59)) {
      fehler = true;
    }
  } else {
    var tag = "-";
...
function cms_vplan_stunde_zusatzstunde(bemerkungszusatz, zwang) {
  ...
  if (!cms_check_ganzzahl(kurs, 0)) {
    fehler = true;
  }
  if (!cms_check_ganzzahl(raum, 0)) {
    fehler = true;
  }
  if (!cms_check_ganzzahl(stunde, 0)) {
    fehler = true;
  }
  if (!cms_check_toggle(anz)) {
    fehler = true;
  }
  if (zwang != "j" && zwang != "n") {
    fehler = true;
  ...
  if (bem.length > 0 && bemerkungszusatz.length > 0) {
    bem = bemerkungszusatz + " - " + bem;
  } else if (bemerkungszusatz.length > 0) {
    bem = bemerkungszusatz;
  }
...
function cms_vplan_stunde_aendern(uid, kid, ort, bemerkungszusatz, zwang, leise, nr) {
  ...
  if (ort == "d" || ort == "k" || ort == "l") {
    ...
    if (!cms_check_ganzzahl(stunde, 0) && stunde != "-") {
      fehler = true;
    }
    if (!cms_check_toggle(anz)) {
      fehler = true;
...
function cms_vplan_auswahl_aendern(modus, aktion, sichtbar) {
  var sichtbar = sichtbar || "1";
  if (cms_check_toggle(sichtbar) && (aktion == "sichtbarkeit" || aktion == "entfall" || aktion == "regelstundenplan" || aktion == "aenderung") && (modus == "k" || modus == "l")) {
    cms_laden_an("Stunden mit der Auswahl bearbeiten", "Die Bearbeitung wird vorbereitet.");
    if (sichtbar == "1") {
      var sichtbarw = "sichtbar";
    } else if (sichtbar == "1") {
      var sichtbarw = "unsichtbar";
    }
    ...
    if (auswahl.length > 0) {
      ...
      if (aktion == "aenderung") {
        neuemeldung += "Vorgemerkte Änderungen der ";
      }
      neuemeldung += "Stunden mit der Auswahl ";
      if (aktion == "sichtbarkeit") {
        neuemeldung += sichtbarw + " machen";
      }
      if (aktion == "entfall") {
        neuemeldung += sichtbarw + " entfallen lassen";
      }
      if (aktion == "regelstundenplan") {
        neuemeldung += "auf den Regelstundenplan zurücksetzen";
      }
      if (aktion == "aenderung") {
        neuemeldung += "entfernen";
      }
      neuemeldung += "</h2>";
      ...
      cms_lehrerdatenbankzugangsdaten_schicken(formulardaten);
      if (aktion != "aenderung") {
        formulardaten.append("uid", std[0]);
      }
      if (aktion != "regelstundenplan") {
        formulardaten.append("kid", std[1]);
      }
      ...
      function anfragennachbehandlung(rueckgabe) {
        ...
        if (stdnr == stdanz) {
          ...
          cms_vplan_alles_neuladen();
          var meldungszusatz = "";
          var meldungsart = "erfolg";
          if (fehlerstunden > 0) {
            meldungszusatz = "<p>Die Anpassung konnt bei " + fehlerstunden + " nicht vorgenommen werden.</p>";
            meldungsart = "info";
          }
...
function cms_groesse_wochenkonf_aendern(ev) {
  ...
  if (spalte_wochenplaene < 15) {
    spalte_wochenplaene = 15;
  }
  if (spalte_wochenplaene > 95) {
    spalte_wochenplaene = 95;
  }
...
function cms_groesse_klasselehrer_aendern(ev) {
  ...
  if (spalte_raeumeplan > 90) {
    spalte_raeumeplan = 90;
  }
  if (spalte_raeumeplan < 5) {
    spalte_raeumeplan = 5;
  }
  if (spalte_klassenplan > 100 - spalte_raeumeplan - 5) {
    spalte_klassenplan = 100 - spalte_raeumeplan - 5;
  }
  if (spalte_klassenplan < 5) {
    spalte_klassenplan = 5;
  }
  if (spalte_klassenplan > 90) {
    spalte_klassenplan = 90;
  }
...
function cms_groesse_lehrerraum_aendern(ev) {
  ...
  if (spalte_lehrerplan > 100 - spalte_klassenplan - 5) {
    spalte_lehrerplan = 100 - spalte_klassenplan - 5;
  }
  if (spalte_lehrerplan < 5) {
    spalte_lehrerplan = 5;
  }
  if (spalte_lehrerplan > 90) {
    spalte_lehrerplan = 90;
  }
