<?php
  ...
  $dbs = cms_verbinden("s");
  $sql = $dbs->prepare("SELECT id, AES_DECRYPT(pinnwand, '$CMS_SCHLUESSEL') FROM sonstigegruppen WHERE LENGTH(AES_DECRYPT(pinnwand, '$CMS_SCHLUESSEL')) > 0 AND (SELECT COUNT(*) FROM sonstigegruppenlinks WHERE gruppe = id) = 0");
  ...
  $sql = $dbs->prepare("INSERT INTO sonstigegruppenlinks (id, gruppe, link, titel, beschreibung) VALUES (?, ?, AES_ENCRYPT(?, '$CMS_SCHLUESSEL'), AES_ENCRYPT('', '$CMS_SCHLUESSEL'), AES_ENCRYPT('', '$CMS_SCHLUESSEL'))");
  $sql->bind_param("iis", $lID, $gID, $link);
  foreach($ins as $i) {
    $lID = cms_generiere_kleinste_id('sonstigegruppenlinks');
    $gID = $i[0];
