...
if ($angemeldet && $zugriff) {
  ...
  if ($sql->execute()) {
    $sql->bind_result($aid, $rid, $grund, $zusatz, $von, $bis);
    while ($sql->fetch()) {
      $a = array();
      $a['aid'] = $aid;
      $a['rid'] = $rid;
  ...
  if (count($IDS) > 0) {
    $vorgabe = implode(',', $IDS);
    $RAEUME = array();
    $sql = "SELECT id, AES_DECRYPT(bezeichnung, '$CMS_SCHLUESSEL') FROM raeume WHERE id IN ($vorgabe)";
    $sql = $dbs->prepare($sql);
    if ($sql->execute()) {
      $sql->bind_result($rid, $bez);
      while ($sql->fetch()) {
        $RAEUME[$rid] = $bez;
      }
    }
    ...
    foreach ($AUSPLANUNGEN AS $a) {
      ...
      $auscode .= "<tr><td>".$RAEUME[$a['rid']]."</td><td>$grund</td><td>$zeitvon</td><td>$zeitbis</td><td>";
        $auscode .= "<span class=\"cms_aktion_klein cms_aktion_nein\" onclick=\"cms_ausplanung_loeschen_anzeigen('".$RAEUME[$a['rid']]."', ".$a['aid'].", 'r', $rueckabwicklung)\"><span class=\"cms_hinweis\">Ausplanung löschen</span><img src=\"res/icons/klein/loeschen.png\"></span>";
      $auscode .= "</td></tr>";
    }
    $code .= "<table class=\"cms_liste\"><tr><th>Räume</th><th>Grund</th><th>von</th><th>bis</th><th>Aktionen</th></tr>$auscode</table>";
  }
  else {$code .= "<p class=\"cms_notiz\">Keine Räume ausgeplant.</p>";}
