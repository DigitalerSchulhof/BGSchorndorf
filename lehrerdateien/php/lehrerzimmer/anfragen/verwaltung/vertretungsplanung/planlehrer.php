...
if (isset($_POST['lehrer'])) 		  {$planziel = $_POST['lehrer'];} 			          else {cms_anfrage_beenden(); exit;}
$planart = 'l';
...
if ($angemeldet && $zugriff) {
  ...
  if ($planziel == '-') {
    ...
    $sql = $dbl->prepare("SELECT von, bis FROM ausplanunglehrer WHERE lehrer = ? AND ((von BETWEEN ? AND ?) OR (bis BETWEEN ? AND ?) OR (von <= ? AND bis >= ?))");
    ...
    $sql1 .= "WHERE unterricht.id NOT IN (SELECT altid FROM unterrichtkonflikt WHERE altid IS NOT null) AND tbeginn >= ? AND tende <= ? AND tlehrer = ?";
    ...
    $sql2 .= "WHERE unterrichtkonflikt.tbeginn >= ? AND unterrichtkonflikt.tende <= ? AND unterrichtkonflikt.tlehrer = ?";
