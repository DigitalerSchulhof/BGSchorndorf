...
if (isset($_POST['raum'])) 		  {$planziel = $_POST['raum'];} 			          else {cms_anfrage_beenden(); exit;}
$planart = 'r';
...
if ($angemeldet && $zugriff) {
  ...
  if ($planziel == '-') {
    ...
    $sql = $dbl->prepare("SELECT von, bis FROM ausplanungraeume WHERE raum = ? AND ((von BETWEEN ? AND ?) OR (bis BETWEEN ? AND ?) OR (von <= ? AND bis >= ?))");
    ...
    $sql1 .= "WHERE unterricht.id NOT IN (SELECT altid FROM unterrichtkonflikt WHERE altid IS NOT null) AND tbeginn >= ? AND tende <= ? AND traum = ?";
    ...
    $sql2 .= "WHERE unterrichtkonflikt.tbeginn >= ? AND unterrichtkonflikt.tende <= ? AND unterrichtkonflikt.traum = ?";
