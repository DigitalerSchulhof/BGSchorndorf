...
if (isset($_POST['zweittag'])) 		{$zweittag = $_POST['zweittag'];} 			        else {cms_anfrage_beenden(); exit;}
if (isset($_POST['zweitmonat'])) 	{$zweitmonat = $_POST['zweitmonat'];} 			    else {cms_anfrage_beenden(); exit;}
if (isset($_POST['zweitjahr'])) 	{$zweitjahr = $_POST['zweitjahr'];} 			      else {cms_anfrage_beenden(); exit;}
if (isset($_POST['planart'])) 		{$planart = $_POST['planart'];} 			          else {cms_anfrage_beenden(); exit;}
if (isset($_POST['planziel'])) 		{$planziel = $_POST['planziel'];} 			        else {cms_anfrage_beenden(); exit;}
if (isset($_POST['regelplan'])) 	{$regelplan = $_POST['regelplan'];} 			      else {cms_anfrage_beenden(); exit;}
...
if (!cms_check_ganzzahl($zweittag,1,31)) {cms_anfrage_beenden(); exit;}
if (!cms_check_ganzzahl($zweitmonat,1,12)) {cms_anfrage_beenden(); exit;}
if (!cms_check_ganzzahl($zweitjahr,0)) {cms_anfrage_beenden(); exit;}
if (!cms_check_toggle($regelplan)) {cms_anfrage_beenden(); exit;}
if (($planart != 'l') && ($planart != 'r') && ($planart != 's') && ($planart != 'k')) {cms_anfrage_beenden(); exit;}
...
if ($angemeldet && $zugriff) {
  ...
  if ($planziel == '-') {
    ...
    $hbz = mktime(0,0,0,$zweitmonat,$zweittag,$zweitjahr);
    $wochentagz = date("N", $hbz);
    $hbz = mktime(0,0,0,$zweitmonat,$zweittag-$wochentagz+1,$zweitjahr);
    $hez = mktime(0,0,0,$zweitmonat,$zweittag-$wochentagz+8,$zweitjahr)-1;
    ...
    // Zeiträume laden
    $ZEITRAEUME = array();
    $sql = $dbs->prepare("SELECT id, beginn, ende, mo, di, mi, do, fr, sa, so, rythmen FROM zeitraeume WHERE (? BETWEEN beginn AND ende) OR (? BETWEEN beginn AND ende) OR (? <= beginn AND ? >= ende) OR (? BETWEEN beginn AND ende) OR (? BETWEEN beginn AND ende) OR (? <= beginn AND ? >= ende) ORDER BY beginn");
    $sql->bind_param("iiiiiiii", $hb, $he, $hb, $he, $hbz, $hez, $hbz, $hez);
    if ($sql->execute()) {
    ...
    if ($planart == 'l') {$sql = $dbl->prepare("SELECT von, bis FROM ausplanunglehrer WHERE lehrer = ? AND ((von BETWEEN ? AND ?) OR (bis BETWEEN ? AND ?) OR (von <= ? AND bis >= ?))");}
    else if ($planart == 'r') {$sql = $dbl->prepare("SELECT von, bis FROM ausplanungraeume WHERE raum = ? AND ((von BETWEEN ? AND ?) OR (bis BETWEEN ? AND ?) OR (von <= ? AND bis >= ?))");}
    ...
    else if ($planart == 's') {$sql = $dbl->prepare("SELECT von, bis FROM ausplanungstufen WHERE stufe = ? AND ((von BETWEEN ? AND ?) OR (bis BETWEEN ? AND ?) OR (von <= ? AND bis >= ?))");}
    ...
    $AUSPLANUNGENR = array();
    $AUSPLANUNGENS = array();
    $sql = "SELECT id, lehrer, grund, von, bis FROM ausplanunglehrer WHERE (von BETWEEN ? AND ?) OR (bis BETWEEN ? AND ?) OR (von <= ? AND bis >= ?) OR (von BETWEEN ? AND ?) OR (bis BETWEEN ? AND ?) OR (von <= ? AND bis >= ?) ORDER BY grund ASC, von ASC, bis DESC";
    $sql = $dbl->prepare($sql);
    $sql->bind_param("iiiiiiiiiiii", $hb, $he, $hb, $he, $hb, $he, $hbz, $hez, $hbz, $hez, $hbz, $hez);
    if ($sql->execute()) {
    ...
    $sql->close();
    $sql = "SELECT id, raum, grund, von, bis FROM ausplanungraeume WHERE (von BETWEEN ? AND ?) OR (bis BETWEEN ? AND ?) OR (von <= ? AND bis >= ?) OR (von BETWEEN ? AND ?) OR (bis BETWEEN ? AND ?) OR (von <= ? AND bis >= ?) ORDER BY grund ASC, von ASC, bis DESC";
    $sql = $dbl->prepare($sql);
    $sql->bind_param("iiiiiiiiiiii", $hb, $he, $hb, $he, $hb, $he, $hbz, $hez, $hbz, $hez, $hbz, $hez);
    if ($sql->execute()) {
      $sql->bind_result($aid, $zid, $grund, $von, $bis);
    ...
    $sql->close();
    $sql = "SELECT id, klasse, grund, von, bis FROM ausplanungklassen WHERE (von BETWEEN ? AND ?) OR (bis BETWEEN ? AND ?) OR (von <= ? AND bis >= ?) OR (von BETWEEN ? AND ?) OR (bis BETWEEN ? AND ?) OR (von <= ? AND bis >= ?) ORDER BY grund ASC, von ASC, bis DESC";
    $sql = $dbl->prepare($sql);
    $sql->bind_param("iiiiiiiiiiii", $hb, $he, $hb, $he, $hb, $he, $hbz, $hez, $hbz, $hez, $hbz, $hez);
    if ($sql->execute()) {
      $sql->bind_result($aid, $zid, $grund, $von, $bis);
    ...
    $sql->close();
    $sql = "SELECT id, stufe, grund, von, bis FROM ausplanungstufen WHERE (von BETWEEN ? AND ?) OR (bis BETWEEN ? AND ?) OR (von <= ? AND bis >= ?) OR (von BETWEEN ? AND ?) OR (bis BETWEEN ? AND ?) OR (von <= ? AND bis >= ?) ORDER BY grund ASC, von ASC, bis DESC";
    $sql = $dbl->prepare($sql);
    $sql->bind_param("iiiiiiiiiiii", $hb, $he, $hb, $he, $hb, $he, $hbz, $hez, $hbz, $hez, $hbz, $hez);
    if ($sql->execute()) {
      $sql->bind_result($aid, $zid, $grund, $von, $bis);
    ...
    if ($regelplan == '0') {
      ...
      $sql = $dbs->prepare("SELECT id AS kid, altid AS uid, tbeginn, tlehrer, traum, tkurs, klasse FROM unterrichtkonflikt LEFT JOIN kurseklassen ON unterrichtkonflikt.tkurs = kurseklassen.kurs AND (tbeginn BETWEEN ? AND ?) OR (tbeginn BETWEEN ? AND ?)");
      $sql->bind_param("iiii", $hb, $he, $hbz, $hez);
      if ($sql -> execute()) {
      ...
      foreach ($AUSPLANUNGENL AS $ap) {
        ...
        $sql = $dbs->prepare($sql);
        $sql->bind_param("iiiiiiiiiiiiii", $ap['von'], $ap['bis'], $ap['von'], $ap['bis'], $ap['von'], $ap['bis'], $ap['zid'], $ap['von'], $ap['bis'], $ap['von'], $ap['bis'], $ap['von'], $ap['bis'], $ap['zid']);
        if ($sql->execute()) {
      ...
      $sql1 .= "WHERE unterricht.id NOT IN (SELECT altid FROM unterrichtkonflikt WHERE altid IS NOT null) AND ((tbeginn >= ? AND tende <= ?) OR (tbeginn >= ? AND tende <= ?))";
      if ($planart == 'l') {$sql1 .= " AND tlehrer = ?";}
      if ($planart == 'r') {$sql1 .= " AND traum = ?";}
      ...
      $sql2 .= "WHERE ((unterrichtkonflikt.tbeginn >= ? AND unterrichtkonflikt.tende <= ?) OR (unterrichtkonflikt.tbeginn >= ? AND unterrichtkonflikt.tende <= ?))";
      if ($planart == 'l') {$sql2 .= " AND unterrichtkonflikt.tlehrer = ?";}
      if ($planart == 'r') {$sql2 .= " AND unterrichtkonflikt.traum = ?";}
      ...
      $sql->bind_param("iiiiiiiiii", $hb, $he, $hbz, $hez, $planziel, $hb, $he, $hbz, $hez, $planziel);
    }
    // REGELSTUNDENPLAN AUSGEBEN
    else {
      $sql = $dbs->prepare("SELECT id AS kid, altid AS uid, tbeginn, tlehrer, traum, tkurs, klasse FROM unterrichtkonflikt LEFT JOIN kurseklassen ON unterrichtkonflikt.tkurs = kurseklassen.kurs AND (tbeginn BETWEEN ? AND ?) OR (tbeginn BETWEEN ? AND ?)");
      $sql->bind_param("iiii", $hb, $he, $hbz, $hez);
      if ($sql -> execute()) {
      ...
      foreach ($LOESUNGEN AS $l) {
        if (count($l['klasse']) > 0) {$lklasse = implode(',', $l['klasse']);}
        $sql = "SELECT COUNT(*) AS anzahl FROM unterricht WHERE pbeginn = ? AND (plehrer = ? OR praum = ?";
        if (count($l['klasse']) > 0) {$sql .= " OR (pkurs IN (SELECT kurs FROM kurseklassen WHERE klasse IN ($lklasse)) AND pkurs NOT IN (SELECT kurs FROM schienenkurse WHERE schiene IN (SELECT schiene FROM schienenkurse WHERE kurs = ?)))";}
        $sql .= ") AND id != ?";
        $sql = $dbs->prepare($sql);
        if (count($l['klasse']) > 0) {
          $sql->bind_param("iiiii", $l['beginn'], $l['lehrer'], $l['raum'], $l['kurs'], $l['uid']);
        }
        else {
          $sql->bind_param("iiii", $l['beginn'], $l['lehrer'], $l['raum'], $l['uid']);
        }
        if ($sql->execute()) {
          $sql->bind_result($anzahl);
          while ($sql->fetch()) {
      ...
      // Konflikte suchen
      foreach ($AUSPLANUNGENL AS $ap) {
        $sql = "SELECT unterricht.id AS uid, IFNULL(unterrichtkonflikt.id, '-') AS kid FROM unterricht LEFT JOIN unterrichtkonflikt ON unterricht.id = unterrichtkonflikt.altid WHERE ((pbeginn BETWEEN ? AND ?) OR (pende BETWEEN ? AND ?) OR (pbeginn <= ? AND pende >= ?)) AND plehrer = ?";
        $sql = $dbs->prepare($sql);
        $sql->bind_param("iiiiiii", $ap['von'], $ap['bis'], $ap['von'], $ap['bis'], $ap['von'], $ap['bis'], $ap['zid']);
        if ($sql->execute()) {
          $sql->bind_result($uid, $kid);
          while ($sql->fetch()) {
            if (!in_array($uid."|".$kid, $KONFLIKTE)) {array_push($KONFLIKTE, $uid."|".$kid);}
      ...
      foreach ($AUSPLANUNGENR AS $ap) {
        $sql = "SELECT unterricht.id AS uid, IFNULL(unterrichtkonflikt.id, '-') AS kid FROM unterricht LEFT JOIN unterrichtkonflikt ON unterricht.id = unterrichtkonflikt.altid WHERE ((pbeginn BETWEEN ? AND ?) OR (pende BETWEEN ? AND ?) OR (pbeginn <= ? AND pende >= ?)) AND praum = ?";
        $sql = $dbs->prepare($sql);
        $sql->bind_param("iiiiiii", $ap['von'], $ap['bis'], $ap['von'], $ap['bis'], $ap['von'], $ap['bis'], $ap['zid']);
        if ($sql->execute()) {
          $sql->bind_result($uid, $kid);
      ...
      foreach ($AUSPLANUNGENK AS $ap) {
        $sql = "SELECT unterricht.id AS uid, IFNULL(unterrichtkonflikt.id, '-') AS kid FROM unterricht LEFT JOIN unterrichtkonflikt ON unterricht.id = unterrichtkonflikt.altid WHERE ((pbeginn BETWEEN ? AND ?) OR (pende BETWEEN ? AND ?) OR (pbeginn <= ? AND pende >= ?)) AND pkurs IN (SELECT kurs FROM kurseklassen WHERE klasse = ?)";
        $sql = $dbs->prepare($sql);
        $sql->bind_param("iiiiiiiiiiiiii", $ap['von'], $ap['bis'], $ap['von'], $ap['bis'], $ap['von'], $ap['bis'], $ap['zid'], $ap['von'], $ap['bis'], $ap['von'], $ap['bis'], $ap['von'], $ap['bis'], $ap['zid']);
        if ($sql->execute()) {
          $sql->bind_result($uid, $kid);
      ...
      foreach ($AUSPLANUNGENS AS $ap) {
        $sql = "SELECT unterricht.id AS uid, IFNULL(unterrichtkonflikt.id, '-') AS kid FROM unterricht LEFT JOIN unterrichtkonflikt ON unterricht.id = unterrichtkonflikt.altid WHERE ((pbeginn BETWEEN ? AND ?) OR (pende BETWEEN ? AND ?) OR (pbeginn <= ? AND pende >= ?)) AND pkurs IN (SELECT id FROM kurse WHERE stufe = ?)";
        $sql = $dbs->prepare($sql);
        $sql->bind_param("iiiiiii", $ap['von'], $ap['bis'], $ap['von'], $ap['bis'], $ap['von'], $ap['bis'], $ap['zid']);
        if ($sql->execute()) {
          $sql->bind_result($uid, $kid);
      ...
      // UNTERRICHT und UNTERRICHTSKONFLIKTLÖSUNGEN laden
      $sql1 = "SELECT unterricht.id AS uid, IFNULL(unterrichtkonflikt.id, '-') AS kid, pkurs, AES_DECRYPT(kurse.bezeichnung, '$CMS_SCHLUESSEL') AS kursbez, AES_DECRYPT(kurse.kurzbezeichnung, '$CMS_SCHLUESSEL') AS kurskurzbez, pbeginn, pende, plehrer, AES_DECRYPT(lehrer.kuerzel, '$CMS_SCHLUESSEL'), ";
      $sql1 .= "AES_DECRYPT(personen.vorname, '$CMS_SCHLUESSEL'), AES_DECRYPT(personen.nachname, '$CMS_SCHLUESSEL'), AES_DECRYPT(personen.titel, '$CMS_SCHLUESSEL'), praum, AES_DECRYPT(raeume.bezeichnung, '$CMS_SCHLUESSEL'), stufen.id AS tstufe, kurseklassen.klasse AS pklasse, '-' AS vplanart, unterricht.tkurs, unterricht.tbeginn, unterricht.tende, unterricht.tlehrer, unterricht.traum ";
      $sql1 .= "FROM unterricht LEFT JOIN kurse ON unterricht.pkurs = kurse.id LEFT JOIN raeume ON unterricht.praum = raeume.id LEFT JOIN lehrer ON unterricht.plehrer = lehrer.id LEFT JOIN personen ON unterricht.plehrer = personen.id ";
      $sql1 .= "LEFT JOIN stufen ON kurse.stufe = stufen.id LEFT JOIN kurseklassen ON unterricht.pkurs = kurseklassen.kurs LEFT JOIN unterrichtkonflikt ON unterricht.id = unterrichtkonflikt.altid ";
      $sql1 .= "WHERE ((pbeginn >= ? AND pende <= ?) OR (pbeginn >= ? AND pende <= ?))";
      if ($planart == 'l') {$sql1 .= " AND plehrer = ?";}
      if ($planart == 'r') {$sql1 .= " AND praum = ?";}
      if ($planart == 'k') {$sql1 .= " AND klasse = ?";}
      if ($planart == 's') {$sql1 .= " AND stufe = ?";}
      $sql = $dbs->prepare("SELECT * FROM (".$sql1.") AS x GROUP BY uid, kid ORDER BY pbeginn, kurskurzbez, kursbez");
      $sql->bind_param("iiiii", $hb, $he, $hbz, $hez, $planziel);
    }
    if ($sql->execute()) {
  ...
  // Plan ausgeben
  if ($regelplan == '1') {$code .= cms_vplan_ausgeben($PLAN, $planart, $AUSPLANUNGENSTD, false, true);}
  else {$code .= cms_vplan_ausgeben($PLAN, $planart, $AUSPLANUNGENSTD, false);}
