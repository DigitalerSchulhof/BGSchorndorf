...
if ($angemeldet && $zugriff) {
  ...
  if ($kid == '-') {
    ...
    $vplanazeigen = null;
    $vplanbemerkung = null;
    $sql = $dbs->prepare("SELECT tkurs, tbeginn, tende, tlehrer, traum, AES_DECRYPT(vplanbemerkung, '$CMS_SCHLUESSEL') FROM unterricht WHERE id = ?");
    $sql->bind_param("i", $uid);
    if ($sql->execute()) {
      $sql->bind_result($kurs, $beginn, $ende, $lehrer, $raum, $vplanbem);
      $sql->fetch();
    } else {$fehler = true;}
    $sql->close();
  ...
  if (!$fehler) {
    // Entfall durchführen
    if ($kid != '-') {
      $sql = $dbs->prepare("UPDATE unterrichtkonflikt SET vplanart = 'e', vplananzeigen = ? WHERE id = ?");
      ...
      $sql = $dbs->prepare("UPDATE unterrichtkonflikt SET altid = ?, tkurs = ?, tbeginn = ?, tende = ?, tlehrer = ?, traum = ?, vplananzeigen = ?, vplanbemerkung = AES_ENCRYPT(?, '$CMS_SCHLUESSEL'), vplanart = 'e' WHERE id = ?");
      $sql->bind_param("iiiiiiisi", $uid, $kurs, $beginn, $ende, $lehrer, $raum, $sichtbar, $vplanbem, $kid);
      $sql->execute();
      $sql->close();
    }
