...
if (cms_angemeldet() && $zugriff) {
  ...
  if (!$fehler) {
    ...
    $sql = "UPDATE unterrichtkonflikt SET tkurs = ?, tbeginn = ?, tende = ?, tlehrer = ?, traum = ?, vplanart = 's', vplananzeigen = '1' WHERE id = ?";
    $sql = $dbs->prepare($sql);
    $sql->bind_param("iiiiii", $auskurs, $neubeginn, $neuende, $auslehrer, $ausraum, $neuid);
    $sql->execute();
    echo "ERFOLG";
  }
  else {echo "FEHLER";}
