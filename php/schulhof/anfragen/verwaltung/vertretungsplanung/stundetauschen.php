...
if (cms_angemeldet() && $zugriff) {
  ...
  if (!$fehler) {
    if (date('d.m.Y', $ausbeginn) == date('d.m.Y', $ziebeginn)) {
      ...
      $art = 'v';
      $ausbem = 'Verlegt vom '.date('d.m.Y', $ausbeginn);
      $ziebem = 'Verlegt vom '.date('d.m.Y', $ziebeginn);
    }
    ...
    if ($zielstundek !== '') {
      ...
      $sql = $dbs->prepare($sql);
      $sql->bind_param("iiiiissi", $ziekurs, $ausbeginn, $ausende, $zielehrer, $zieraum, $art, $ziebem, $zielstundek);
      $sql->execute();
    }
    else {
      ...
      $sql = $dbs->prepare($sql);
      $sql->bind_param("iiiiiissi", $zielstundeu, $ziekurs, $ausbeginn, $ausende, $zielehrer, $zieraum, $art, $ziebem, $neuid);
      $sql->execute();
    }
    ...
    if ($ausgangsstundek !== '') {
      $sql = "UPDATE unterrichtkonflikt SET tkurs = ?, tbeginn = ?, tende = ?, tlehrer = ?, traum = ?, vplanart = ?, vplananzeigen = '1', vplanbemerkung = AES_ENCRYPT(?, '$CMS_SCHLUESSEL') WHERE id = ?";
      $sql = $dbs->prepare($sql);
      $sql->bind_param("iiiiissi", $auskurs, $ziebeginn, $zieende, $auslehrer, $ausraum, $art, $ausbem, $ausgangsstundek);
      $sql->execute();
    }
    else {
      $neuid = cms_generiere_kleinste_id("unterrichtkonflikt");
      $sql = "UPDATE unterrichtkonflikt SET altid = ?, tkurs = ?, tbeginn = ?, tende = ?, tlehrer = ?, traum = ?, vplanart = ?, vplananzeigen = '1', vplanbemerkung = AES_ENCRYPT(?, '$CMS_SCHLUESSEL') WHERE id = ?";
      $sql = $dbs->prepare($sql);
      $sql->bind_param("iiiiiissi", $ausgangsstundeu, $auskurs, $ziebeginn, $zieende, $auslehrer, $ausraum, $art, $ausbem, $neuid);
      $sql->execute();
    }
    echo "ERFOLG";
  }
