...
if (cms_angemeldet() && r("schulhof.planung.räume.anlegen")) {
	$fehler = false;
	...
	if (!$fehler) {
		...
		$sql = $dbs->prepare("SELECT COUNT(id) AS anzahl FROM raeume WHERE bezeichnung = AES_ENCRYPT(?, '$CMS_SCHLUESSEL')");
	...
	if (!$fehler) {
		...
		$id = cms_generiere_kleinste_id('raeume');
		// Raum EINTRAGEN
		$dbs = cms_verbinden('s');
	  ...
	  $sql->close();
		...
		$sql = $dbs->prepare("UPDATE raeumegeraete SET bezeichnung = AES_ENCRYPT(?, '$CMS_SCHLUESSEL'), standort = ?, statusnr = 0 WHERE id = ?");
