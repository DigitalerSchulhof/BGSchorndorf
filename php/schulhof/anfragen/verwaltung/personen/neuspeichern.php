...
$zugriff = $CMS_RECHTE['Personen']['Personen anlegen'];
...
if (cms_angemeldet() && $zugriff) {
	...
	if (!$fehler) {
	  ...
	  $sql->close();
		...
		$sql = $dbs->prepare("INSERT INTO personen_einstellungen (person, notifikationsmail, postmail, postalletage, postpapierkorbtage, vertretungsmail, uebersichtsanzahl, oeffentlichertermin, oeffentlicherblog, oeffentlichegalerie, inaktivitaetszeit) VALUES (?, AES_ENCRYPT('0', '$CMS_SCHLUESSEL'), AES_ENCRYPT('1','$CMS_SCHLUESSEL'), AES_ENCRYPT('365','$CMS_SCHLUESSEL'), AES_ENCRYPT('30','$CMS_SCHLUESSEL'), AES_ENCRYPT('1','$CMS_SCHLUESSEL'), AES_ENCRYPT('5','$CMS_SCHLUESSEL'), AES_ENCRYPT('1','$CMS_SCHLUESSEL'), AES_ENCRYPT('1','$CMS_SCHLUESSEL'), AES_ENCRYPT('1','$CMS_SCHLUESSEL'), AES_ENCRYPT('30','$CMS_SCHLUESSEL'));");
		$sql->bind_param("i", $id);
		$sql->execute();
		$sql->close();
