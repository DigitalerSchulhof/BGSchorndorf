...
if (cms_angemeldet() && $zugriff) {
	...
	$sql = $dbs->prepare("INSERT INTO kursemitglieder (gruppe, person, dateiupload, dateidownload, dateiumbenennen, termine, blogeintraege, chatten, chattenab) SELECT kurs, person, dateiupload, dateidownload, dateiumbenennen, termine, blogeintraege, chatten, chattenab FROM klassenmitglieder JOIN kurseklassen ON klassenmitglieder.gruppe = kurseklassen.klasse JOIN klassen ON kurseklassen.klasse = klassen.id JOIN personen ON personen.id = person WHERE schuljahr = ? AND art = AES_ENCRYPT('s', '$CMS_SCHLUESSEL')");
