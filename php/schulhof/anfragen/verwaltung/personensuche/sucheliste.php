		...
		if (strlen($sqlwhere) > 0) {
			...
			if ($sql->execute()) {
				...
				while ($sql->fetch()) {
					...
					$ausgabe .= "<td>";
					if (r("schulhof.verwaltung.nutzerkonten.anmeldedetails")) {
						if (!is_null($pnutzer)) {
					...
					if (r("schulhof.verwaltung.personen.daten")) {
					...
					$zugriff = (($part == 's') || ($part == 'e')) && r("schulhof.verwaltung.personen.schülereltern");
					if ($zugriff) {
					...
					$zugriff = ($part == 'l') && r("schulhof.verwaltung.lehrer.kürzel");
					if ($zugriff) {
					...
					$zugriff = is_null($pnutzer) && r("schulhof.verwaltung.nutzerkonten.anlegen");
					if ($zugriff) {
					...
					$zugriff = !is_null($pnutzer) && r("schulhof.verwaltung.nutzerkonten.bearbeiten");
					if ($zugriff) {
					...
					$zugriff = !is_null($pnutzer) && r("schulhof.verwaltung.nutzerkonten.löschen");
					if ($zugriff) {
