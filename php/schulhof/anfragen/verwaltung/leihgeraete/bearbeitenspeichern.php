...
if (isset($_SESSION["LEIHGERAETBEARBEITEN"])) {$id = $_SESSION["LEIHGERAETBEARBEITEN"];} else {echo "FEHLER"; exit;}
...
if (cms_angemeldet() && r("schulhof.organisation.leihgeräte.bearbeiten")) {
	$fehler = false;
	...
	if (!$fehler) {
		...
		$sql = $dbs->prepare("SELECT COUNT(id) AS anzahl FROM leihen WHERE bezeichnung = AES_ENCRYPT(?, '$CMS_SCHLUESSEL') AND id != ?");
	...
	if (!$fehler) {
	  ...
	  $sql->close();
		...
		$gids = explode('|', $geraeteids);
		$sqlneu = $dbs->prepare("UPDATE leihengeraete SET bezeichnung = AES_ENCRYPT(?, '$CMS_SCHLUESSEL'), standort = ? WHERE id = ?");
		$sqlalt = $dbs->prepare("UPDATE leihengeraete SET bezeichnung = AES_ENCRYPT(?, '$CMS_SCHLUESSEL') WHERE id = ?");
		for ($i=1; $i<count($gids); $i++) {
		...
		if (strlen($gidsvorhanden) > 1) {
			...
			$sql = $dbs->prepare("DELETE FROM leihengeraete WHERE id = ? AND standort = ?");
		...
		$sql = $dbs->prepare("DELETE FROM leihenblockieren WHERE standort = ?");
	  $sql->bind_param("i", $id);
	  $sql->execute();
	  $sql->close();
