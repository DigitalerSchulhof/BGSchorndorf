...
if (cms_angemeldet() && r("schulhof.information.dauerbrenner.bearbeiten")) {
	$fehler = false;
	...
	if (!$fehler) {
		...
		$sql = $dbs->prepare("SELECT COUNT(id) AS anzahl FROM dauerbrenner WHERE bezeichnung = AES_ENCRYPT(?, '$CMS_SCHLUESSEL') AND id != ?");
