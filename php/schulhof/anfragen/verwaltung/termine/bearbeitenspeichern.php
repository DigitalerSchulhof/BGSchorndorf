...
if (cms_angemeldet() && $zugriff) {
  ...
  $sql = $dbs->prepare("SELECT COUNT(*) as anzahl FROM termine WHERE bezeichnung = AES_ENCRYPT(?, '$CMS_SCHLUESSEL') AND (beginn BETWEEN ? AND ?) AND id != ?");
  ...
  if ($downloadanzahl > 0) {
    ...
    $sql->close();
		...
		foreach($CMS_GRUPPEN as $g) {
			$ids = str_replace('|', ',', $zugeordnet[$g]);
      $gk = cms_textzudb($g);
      $sql = $dbs->prepare("DELETE FROM $gk"."termine WHERE termin = ?");
    ...
    $sql = $dbs->prepare("DELETE FROM terminedownloads WHERE termin = ?");
    $sql->bind_param("i", $terminid);
    $sql->execute();
    $sql->close();
