...
if (cms_angemeldet() && $zugriff) {
  ...
  if (!cms_check_toggle($notifikationen)) {$fehler = true;}
	...
	$sql = $dbs->prepare("SELECT COUNT(*) as anzahl FROM $gk"."blogeintraegeintern WHERE bezeichnung = AES_ENCRYPT(?, '$CMS_SCHLUESSEL') AND datum = ?");
  ...
  if ($beschluesseanzahl > 0) {
    ...
    $eintrag['link']      = "Schulhof/Gruppen/$gruppensj/".cms_textzulink($gruppe)."/$gruppenbez/Blog/$jahr/$monatsname/$tag/".cms_textzulink($bezeichnung);
		...
		if($notifikationen && ($aktiv == 1)) {
			// ToDo Eintragen
			$sql = $dbs->prepare("INSERT INTO {$gk}todoartikel (person, blogeintrag, termin) SELECT abo.person, ?, NULL FROM {$gk}notifikationsabo as abo WHERE abo.gruppe = ? AND abo.person != ? AND NOT EXISTS(SELECT todo.blogeintrag FROM {$gk}todoartikel as todo WHERE todo.person = abo.person AND todo.blogeintrag = ?)");
