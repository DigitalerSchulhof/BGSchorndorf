<?php
function cms_leihgeraet_ausgeben ($leihgeraetid) {
	...
	if ($leihgeraetid != "-") {
		...
		$sql = $dbs->prepare("SELECT AES_DECRYPT(bezeichnung, '$CMS_SCHLUESSEL') AS bezeichnung, buchbar, verfuegbar, externverwaltbar FROM leihen WHERE id = ?");
	  $sql->bind_param("i", $leihgeraetid);
	  if ($sql->execute()) {
	    $sql->bind_result($bezeichnung, $buchbar, $verfuegbar, $extern);
	    $sql->fetch();
	  }
	  $sql->close();

		// Geräte laden
		$sql = $dbs->prepare("SELECT * FROM (SELECT id, AES_DECRYPT(bezeichnung, '$CMS_SCHLUESSEL') AS bezeichnung FROM leihengeraete WHERE standort = ?) AS x ORDER BY bezeichnung ASC");
	  $sql->bind_param("i", $leihgeraetid);
	  if ($sql->execute()) {
			$i = 1;
