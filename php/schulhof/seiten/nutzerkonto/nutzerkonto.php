...
foreach($CMS_GRUPPEN as $g) {
	$gk = cms_textzudb($g);
	$sql .= "(SELECT 'b', AES_DECRYPT(t.bezeichnung, '$CMS_SCHLUESSEL') as tbez, AES_DECRYPT(t.beschreibung, '$CMS_SCHLUESSEL'), IFNULL(AES_DECRYPT(s.bezeichnung, '$CMS_SCHLUESSEL'), 'Schuljahrübergreifend'), '$g' as gr, g.id as gid, AES_DECRYPT(g.bezeichnung, '$CMS_SCHLUESSEL'), a.id, AES_DECRYPT(a.bezeichnung, '$CMS_SCHLUESSEL') as abez, a.datum FROM {$gk}blogeintraegeintern as a JOIN {$gk}todoartikel as t ON t.blogeintrag = a.id JOIN $gk as g ON g.id = a.gruppe LEFT JOIN schuljahre as s ON s.id = g.schuljahr WHERE person = $CMS_BENUTZERID ORDER BY gr, gid, tbez, abez) UNION ";
	$sql .= "(SELECT 't', AES_DECRYPT(t.bezeichnung, '$CMS_SCHLUESSEL') as tbez, AES_DECRYPT(t.beschreibung, '$CMS_SCHLUESSEL'), IFNULL(AES_DECRYPT(s.bezeichnung, '$CMS_SCHLUESSEL'), 'Schuljahrübergreifend'), '$g' as gr, g.id as gid, AES_DECRYPT(g.bezeichnung, '$CMS_SCHLUESSEL'), a.id, AES_DECRYPT(a.bezeichnung, '$CMS_SCHLUESSEL') as abez, a.beginn FROM {$gk}termineintern as a JOIN {$gk}todoartikel as t ON t.termin = a.id JOIN $gk as g ON g.id = a.gruppe LEFT JOIN schuljahre as s ON s.id = g.schuljahr WHERE person = $CMS_BENUTZERID ORDER BY gr, gid, tbez, abez) UNION ";
}
$sql = substr($sql, 0, -6);
$sql = $dbs->prepare($sql);
// t termin
